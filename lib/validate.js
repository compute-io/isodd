...
function validate( opts ) {
	if ( !isObject( opts ) ) {
		return new TypeError( 'isodd()::invalid input argument. Options argument must be an object. Value: `' + opts + '`.' );
	}
	if ( opts.hasOwnProperty( 'copy' ) ) {
		if ( !isBoolean( opts.copy ) ) {
			return new TypeError( 'isodd()::invalid option. Copy option must be a boolean primitive. Option: `' + opts.copy + '`.' );
		}
	} else {
		opts.copy = true;
	}
	if ( opts.hasOwnProperty( 'accessor' ) ) {
		if ( !isFunction( opts.accessor ) ) {
			return new TypeError( 'isodd()::invalid option. Accessor must be a function. Option: `' + opts.accessor + '`.' );
		}
	} else {
		opts.accessor = null;
	}
